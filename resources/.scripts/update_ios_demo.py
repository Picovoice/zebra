# Copyright 2026 Picovoice Inc.
#
# You may not use this file except in compliance with the license. A copy of the license is located in the "LICENSE"
# file accompanying this source.
#
# Unless required by applicable law or agreed to in writing, software distributed under the License is distributed on
# an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the License for the
# specific language governing permissions and limitations under the License.
#

import json
import os

from xml.dom import minidom


def update_ios_demo(models):

    scheme_dir = os.path.join(
        os.path.dirname(__file__),
        "../../demo/ios/ZebraDemo/ZebraDemo.xcodeproj/xcshareddata/xcschemes")
    base_scheme = os.path.join(
        os.path.dirname(__file__),
        scheme_dir,
        "enfrDemo.xcscheme")

    for model in models:
        source, target = model.removeprefix("zebra_params_").removesuffix(".pv").split("_")
        if source == 'en' and target == 'fr':
            continue

        model_target = source + target
        model_scheme = os.path.join(scheme_dir, f"{model_target}Demo.xcscheme")
        if not os.path.exists(model_scheme):
            print(f"Creating iOS demo scheme for `{model_target}`")
            base_scheme_content = minidom.parse(base_scheme)
            pre_build_action = base_scheme_content.getElementsByTagName('ActionContent')[0]
            env_var = base_scheme_content.getElementsByTagName('EnvironmentVariable')[0]
            pre_build_action.setAttribute(
                'scriptText',
                pre_build_action.attributes['scriptText'].value.replace(" en_fr", f" {source}_{target}"))
            env_var.setAttribute('value', f'{source}_{target}')
            with open(os.path.join(scheme_dir, f"{model_target}Demo.xcscheme"), 'w') as f:
                f.write(base_scheme_content.toxml())
        else:
            print(f"iOS scheme for `{model_target}` already exists")


def main():
    with open(os.path.join(os.path.dirname(__file__), "../.test/test_data.json"), encoding='utf-8') as f:
        json_content = json.loads(f.read())

    models = list()
    for translation_tests in json_content['tests']['translation_tests']:
        source = translation_tests["source"]
        for target in translation_tests['translations'].keys():
            models.append(f'zebra_params_{source}_{target}.pv')

    update_ios_demo(models=models)


if __name__ == '__main__':
    main()
